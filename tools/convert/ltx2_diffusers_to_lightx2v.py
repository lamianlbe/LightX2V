...
# The official single-file releases carry a 5-block config plus a version
# marker: {vae, audio_vae, vocoder, transformer, scheduler} and
# model_version="2.3.0". Nothing currently reads the scheduler block on the
# LTX-2.3 path (only ltx2_5 reads model_version), but emitting the same shape
# keeps a converted file interchangeable with an official one.
OPTIONAL_CONFIG_COMPONENTS = ["scheduler"]
...
def build_main_metadata(src: Path, reference: Optional[Path] = None) -> Dict[str, str]:
    ...
    # Present in an official release; a diffusers export may or may not have it.
    for name in OPTIONAL_CONFIG_COMPONENTS:
        for candidate in (src / name / "config.json", src / name / f"{name}_config.json"):
            if candidate.exists():
...
def main() -> int:
    ...
    # Two ready-to-run configs, both copied from the upstream references with
    # only the deltas a converted model on a server GPU needs. Kept faithful on
    # purpose: mm_guider scales, sample_shift, infer_steps and the sigma lists
    # are upstream's tuned values and are not ours to second-guess.
    #
    # The deltas, and the whole of them:
    #   attn_type            sage_attn2 -> flash_attn4. SageAttention 2 has no
    #                        sm100/sm103 kernels, so it cannot load on B200/B300;
    #                        FA4 covers Hopper and Blackwell both.
    #   dit_original_ckpt    the merged file this script just wrote
    #   gemma_original_ckpt  set explicitly, because the default resolution
    #                        rglobs model_path for model*.safetensors and that is
    #                        ambiguous once other checkpoints sit alongside it
    #   upsampler_*          pointed at the x2 file this script wrote
    #
    # config.json carries the pipeline geometry (vae_scale_factors, audio_*),
    # matching what Lightricks/LTX-2/transformer/config.json supplies for an
    # official checkpoint. That is why neither config repeats it.
    common = {
    ...
    # Profile 1: mirrors configs/ltx2/ltx2_3_i2av_keyframes.json -- the dev
    # model with CFG and the multimodal guider, 30 steps, single stage.
    cfg_dev = {
        "_comment": f"From {args.src} via ltx2_diffusers_to_lightx2v.py. Mirrors upstream configs/ltx2/ltx2_3_i2av_keyframes.json; only attn_type and the checkpoint paths differ. Use with --model_path {args.out}.",
        "infer_steps": 30,
        ...
        "mm_guider": {
            "enabled": True,
            "video": {"cfg_scale": 3.0, "stg_scale": 1.0, "stg_blocks": [28], "modality_scale": 3.0, "rescale_scale": 0.7, "skip_step": 0},
            "audio": {"cfg_scale": 7.0, "stg_scale": 1.0, "stg_blocks": [28], "modality_scale": 3.0, "rescale_scale": 0.7, "skip_step": 0},
        },
        **common,
    }

    # Profile 2: mirrors configs/ltx2/ltx2_3_distill_upsample_offload.json --
    # the distilled model, 8 steps, no CFG, two-stage with the x2 upscaler.
    cfg_distill = {
        "_comment": f"From {args.src} via ltx2_diffusers_to_lightx2v.py. Mirrors upstream configs/ltx2/ltx2_3_distill_upsample_offload.json; only attn_type and the checkpoint paths differ. Requires a DISTILLED checkpoint -- the sigma list below is the distilled schedule and is wrong for a dev model. Use with --model_path {args.out}.",
        "infer_steps": 8,
    ...
    written = []
    for filename, payload in (("lightx2v_ltx2_3_dev.json", cfg_dev), ("lightx2v_ltx2_3_distill_upsample.json", cfg_distill)):
        path = args.out / filename
        with open(path, "w") as f:
            json.dump(payload, f, indent=4)
        written.append(path)
        print(f"Wrote {path}")
    run_cfg_path = written[0]

    status = verify(args.out, args.name)
    if status == 0:
        print(
            "\nRun with (pick the profile matching your checkpoint):\n"
            f"  python -m lightx2v.infer --model_cls ltx2 --task t2av \\\n"
            f"      --model_path {args.out} \\\n"
            f"      --config_json {written[0]} \\\n"
            f'      --prompt "..." --save_result_path out.mp4\n'
            f"\n  dev checkpoint     -> {written[0].name}  (30 steps, CFG + mm_guider)\n"
            f"  distilled checkpoint -> {written[1].name}  (8 steps, no CFG, x2 upscaler)"
        )
    return status
